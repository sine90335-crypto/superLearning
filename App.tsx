...
const App: React.FC = () => {
          ...
          if (p.id === planId) {
            ...
            if (task?.googleTaskId && task?.googleTaskListId && googleToken) {
              updateTaskStatus(
                ...
                task.googleTaskId,
                newlyDone ? "completed" : "needsAction",
              ).catch((err) => {
            ...
            return {
              ...p,
              daily: p.daily.map((t) =>
                t.id === taskId ? { ...t, done: !t.done, isActive: false } : t,
              ),
            };
          }
          return p;
  ...
  const updateTask = useCallback(
    (planId: string, taskId: string, updates: Partial<PlanTask>) => {
      setPlans((prev) =>
        prev.map((p) => {
          if (p.id === planId) {
            return {
  ...
  return (
    ...
    >
      {/* Dynamic Background Orbs for Glassmorphism Context */}
      <div className="absolute top-[-20%] left-[-10%] w-[50vw] h-[50vw] bg-apple-blue/20 rounded-full blur-[140px] pointer-events-none z-0"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[45vw] h-[45vw] bg-apple-purple/15 rounded-full blur-[120px] pointer-events-none z-0"></div>

      <nav
        className={`${isSidebarCollapsed ? "w-16" : "w-16 md:w-64"} flex flex-col border-r z-30 transition-all duration-300 backdrop-blur-2xl`}
        style={{
      ...
      >
        <div className="p-4 md:p-6 mb-4 flex items-center justify-between gap-2.5">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 bg-gradient-to-b from-[#1c1c1e] to-black border border-white/10 rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0 shadow-lg">
              s
            </div>
            {!isSidebarCollapsed && (
              <span className="hidden md:block font-bold text-[15px] tracking-tight text-zinc-100 truncate">
                SILI Intelligence
              </span>
            )}
          </div>
          ...
          >
            <i
              className={`fas ${isSidebarCollapsed ? "fa-angle-right" : "fa-angle-left"} text-xs`}
            ></i>
          </button>
        </div>
        <div className="flex-1 px-3 space-y-1">
          {[
            ...
            return (
              ...
              >
                <i
                  className={`fas ${item.icon} w-5 text-sm ${isActive ? "text-apple-blue" : "text-zinc-500 group-hover:text-zinc-300"}`}
                ></i>
                {!isSidebarCollapsed && (
        ...
        <div className="p-4 mt-auto">
          {isSidebarCollapsed ? (
            ...
            <div
              className="hidden md:block backdrop-blur-xl border rounded-2xl p-4 shadow-xl"
              style={{
            ...
            >
              ...
              <div className="h-[3px] w-full bg-zinc-800 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-apple-blue to-[#30d158] shadow-[0_0_10px_rgba(10,132,255,0.4)] transition-all duration-500"
                  style={{ width: `${(stats.xp % 500) / 5.0}%` }}
                ></div>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
          {view === AppView.DASHBOARD && (
            <div className="max-w-7xl mx-auto space-y-8">
              <section
                className="backdrop-blur-3xl border p-8 md:p-12 rounded-[28px] relative overflow-hidden shadow-2xl"
                style={{
                  backgroundColor: "var(--glass-bg)",
                  borderColor: "var(--border-color)",
                }}
              >
                ...
                <div className="absolute top-0 right-0 p-10 opacity-5 rotate-12 transition-transform duration-1000 hidden md:block">
                  <i
                    className="fas fa-brain text-[120px]"
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-zinc-400 max-w-lg mb-8 leading-relaxed text-xs">
                    基于你最近的学习笔记或PDF内容，SILI
                    将深度进行全流程的重点推演，为您安排科学的学习周期、宏观路线图与高并发记忆测试。
                  </p>
                  <div className="flex flex-col md:flex-row gap-3.5 max-w-4xl">
                    ...
                    <button
                      ...
                      className="px-8 py-4 bg-white text-black hover:bg-zinc-200 rounded-2xl font-bold text-xs tracking-tight transition-all duration-200 flex items-center justify-center gap-2 shadow-xl active:scale-95"
                    >
            ...
            <div className="max-w-7xl mx-auto h-full">
              {plans.length > 0 ? (
                <LearningPlanView
                  ...
                  onUpdateTask={(tid, updates) =>
                    updateTask(plans[0].id, tid, updates)
                  }
                  user={user}
                ...
                <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4">
                  <i
                    className="fas fa-calendar-check text-4xl"
                  ...
                  <button
                    ...
                    className="mt-4 px-6 py-2 bg-apple-blue text-white font-bold text-sm rounded-xl hover:bg-blue-600 transition"
                  >
                    去控制台生成
                  </button>
                </div>
            ...
            <div className="h-[calc(100vh-160px)] flex gap-6">
              <div className="flex-1">
                <Editor
                  ...
                  onGenerateCards={async (c) => {
                    setLoadingText("提取卡片中...");
                    setIsGenerating(true);
                    try {
                      const newCards = await generateFlashcardsFromNote(
                        c,
                        activeNoteId,
                      );
                  ...
                  onGenerateQuiz={async (c) => {
                    setLoadingText("准备题目中...");
                    setIsGenerating(true);
                    try {
                      const qs = await generateQuiz(c);
                      setQuizQuestions(qs);
                  ...
                  onStartDemo={async (c) => {
                    setLoadingText("逻辑推演中...");
                    setIsGenerating(true);
                    try {
                      const steps = await generateDynamicDemo(c);
                      setDemoSteps(steps);
                      setView(AppView.DEMO);
                    } finally {
                      setIsGenerating(false);
                    }
                  }}
          ...
          {view === AppView.QUIZ && (
            <QuizView
              ...
              onClose={() => setView(AppView.EDITOR)}
              onComplete={() => setView(AppView.REWARD)}
            />
          )}
          {view === AppView.REWARD && (
            <RewardView
      ...
      {isGenerating && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-2xl z-[100] flex flex-col items-center justify-center animate-in fade-in duration-300">
          ...
          <p className="text-lg font-semibold text-white tracking-tight animate-pulse">
            {loadingText}
          </p>
          <p className="mt-3 text-zinc-500 text-xs font-mono">
            SILI System Synapse Optimizing...
          </p>
        </div>
      )}
    </div>
