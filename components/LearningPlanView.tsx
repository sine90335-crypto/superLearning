      ...
      >
        ...
        <div className="flex-1 min-w-0" onClick={() => onToggle(task.id)}>
          <div className="flex justify-between items-start">
            <span
              className={`text-xs font-semibold tracking-tight truncate block ${task.done ? "text-zinc-500 line-through" : "text-zinc-200"}`}
            >
              {task.time && (
                <span className="text-apple-blue mr-2 font-mono">
                  [{task.time}]
                </span>
              )}
              {task.task}
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {task.done && (
              <span className="text-[8px] text-apple-green font-extrabold uppercase tracking-widest">
                Done
              </span>
            )}
          </div>
          ...
          {!task.done && (
            <>
              {task.remainingSeconds !== undefined && (
                <span
                  className={`font-mono text-xs font-bold w-12 text-center ${task.isActive ? "text-apple-blue" : "text-zinc-500"}`}
                >
              ...
              >
                <i
                  className={`fas ${task.isActive ? "fa-pause" : "fa-play"} text-[10px]`}
                ></i>
              </button>
            </>
          )}
...
export const LearningPlanView: React.FC<LearningPlanViewProps> = ({
  ...
  const [activeIntegrationChannel, setActiveIntegrationChannel] = useState<
    "tasks" | "calendar" | "docs" | "sheets" | "forms" | "chat" | "keep"
  >("tasks");
  ...
  const [activeWidgetTab, setActiveWidgetTab] = useState<"sync" | "browser">(
    "sync",
  );
  ...
  const [createdForms, setCreatedForms] = useState<
    Array<{ id: string; title: string; responderUri: string }>
  >(() => {
    ...
    return [
      {
        ...
        content:
          "因为消费者端的 Google Keep API 受到 Google Workspace 域权限严格限制。为了让您在 AI 个人学习中拥有同等便捷的体验，SILI 深度适配并启动了这款本地 Keep 精装工作板。支持便签盯、五彩画布和极速闪存记录！",
        color: "#fef08a",
        pinned: true,
      },
      {
        ...
        title: "💡 脑电波灵感栏",
        content:
          "在这里你可以记录学习过程中稍纵即逝的灵感或重难点疑难排查。右下角的按钮可以进行便签颜色自定义哦！",
        color: "#bbf7d0",
        pinned: false,
      },
    ];
  ...
  const stats = useMemo(() => {
    const totalUnits = plan.daily.reduce(
      (acc, curr) => acc + (curr.units || 0),
      0,
    );
    const completedUnits = plan.daily
      .filter((t) => t.done)
      .reduce((acc, curr) => acc + (curr.units || 0), 0);
  ...
  const handlePushSync = async () => {
    ...
    try {
      ...
      for (const task of plan.daily) {
        let matchingGTask = existingGoogleTasks.find(
          (gt) => gt.id === task.googleTaskId,
        );
        if (!matchingGTask) {
          matchingGTask = existingGoogleTasks.find(
            (gt) => gt.title === task.task,
          );
        }
        ...
        if (matchingGTask) {
          ...
          if (
            task.googleTaskId !== matchingGTask.id ||
            task.googleTaskListId !== targetListId
          ) {
  ...
  const handlePullSync = async () => {
    if (!googleToken) return;
    const targetTaskListId = plan.daily.find(
      (t) => !!t.googleTaskListId,
    )?.googleTaskListId;
    if (!targetTaskListId) {
    ...
    try {
      ...
      for (const localTask of plan.daily) {
        if (localTask.googleTaskId) {
          const cloudTask = cloudTasks.find(
            (ct) => ct.id === localTask.googleTaskId,
          );
          if (cloudTask) {
            const isCloudCompleted = cloudTask.status === "completed";
            if (isCloudCompleted !== localTask.done) {
              onUpdateTask(localTask.id, {
                done: isCloudCompleted,
                isActive: false,
              });
              localUpdates++;
            }
      ...
      setWidgetStatusMsg(
        `✅ 进度同步：成功拉取并校验了 ${localUpdates} 个任务状态！`,
      );
      setTimeout(() => setWidgetStatusMsg(""), 4500);
    } catch (err: any) {
      console.error(err);
  ...
  const handleDeleteBrowserTask = async (taskId: string) => {
    ...
    const confirmed = window.confirm(
      "您确定要从 Google Tasks Cloud 中彻底删除该任务吗？此操作无法撤销。",
    );
    if (!confirmed) return;
  ...
  const handleScheduleTaskOnCalendar = async (task: PlanTask) => {
    ...
    const confirmed = window.confirm(
      `您确定要为任务 “${task.task}” 在 Google 日历中创建一个 30 分钟的学习日程吗？`,
    );
    if (!confirmed) return;
  ...
  const handleExportToGoogleDocs = async () => {
    ...
    try {
      ...
      setCreatedDocs((prev) => [
        { id: doc.documentId, title: docTitle, url: docUrl },
        ...prev,
      ]);
      setDocExportStatus("✅ 文档已完成云端转录并安全发布！");

      const confirmOpen = window.confirm(
        `导出成功！文档《${docTitle}》已保存在您的谷歌云端硬盘。\n是否现在就在新窗口中打开它进行浏览？`,
      );
      if (confirmOpen) {
  ...
  const handleExportToGoogleSheets = async () => {
    ...
    try {
      ...
      setSheetExportStatus(
        "🖋️ 正在往 Google Sheets 导入进化计划维度与核心序列...",
      );
      ...
      setCreatedSheets((prev) => [
        { id: spreadsheet.spreadsheetId, title: sheetTitle, url: sheetUrl },
        ...prev,
      ]);
      setSheetExportStatus("✅ 进化数据表已成功建立并完成初始映射！");

      const confirmOpen = window.confirm(
        `进度表《${sheetTitle}》已保存在您的谷歌云盘！\n是否立即在新窗口中打开它？`,
      );
      if (confirmOpen) {
  ...
  const handleAddHoursRowToSheets = async (e: React.FormEvent) => {
    ...
    const confirmed = window.confirm(
      `您确定要向 Google Sheet《${currentSheet.title}》追加一条学习日志与专注时长记录吗？`,
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setSheetExportStatus(
        "✅ 学习心得与耗时已追加并在 Google Sheet 安全落盘！",
      );
  ...
  const handleCreateEvaluationForm = async () => {
    ...
    try {
      ...
      setFormCreateStatus(
        "🖋️ 正在生成复盘问卷多维度指标并进行批量注入 (batchUpdate)...",
      );
      const questions = [
        {
        ...
        {
          title:
            "请写下任何您对学习方案的纠偏建议，或您希望 SILI 陪伴您做出的改变：",
        },
      ];
      ...
      setCreatedForms((prev) => [
        { id: form.formId, title: formTitle, responderUri },
        ...prev,
      ]);
      setFormCreateStatus("✅ 自测问卷已成功发布并激活响应收集接口！");

      const confirmOpen = window.confirm(
        `评估反馈表《${formTitle}》架设成功！已经自动与您的谷歌账号表单无缝挂接。\n是否现在就在新窗口中打开表单进行模拟评估与答题？`,
      );
      if (confirmOpen) {
  ...
  const handleSendFullProgressAlert = async () => {
    ...
    const progressPercent = Math.round(
      (stats.completedUnits / (stats.totalUnits || 1)) * 100,
    );
      ...
      <div className="flex-1 space-y-6 overflow-y-auto pr-2 pb-32 custom-scrollbar">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-[#1c1c1e] border border-white/5 p-4 rounded-xl text-center shadow">
            ...
            <div className="text-xl font-bold font-mono text-apple-blue">
              {stats.totalUnits}
            </div>
          </div>
          <div className="bg-[#1c1c1e] border border-white/5 p-4 rounded-xl text-center shadow">
            ...
            <div className="text-xl font-bold font-mono text-apple-green">
          ...
          <div className="bg-[#1c1c1e] border border-white/5 p-4 rounded-xl text-center shadow">
            ...
            <div className="text-xs font-bold font-mono text-apple-purple mt-1.5">
            ...
            {user && (
              <div className="flex items-center gap-2 shrink-0">
                {user.photoURL ? (
                  ...
                  <div className="w-5 h-5 bg-apple-blue/20 rounded-full flex items-center justify-center text-[9px] font-bold text-apple-blue">
                    <i className="fas fa-user text-[8px]"></i>
                  </div>
                )}
                <span className="text-[10px] text-zinc-300 font-medium truncate max-w-[80px] hidden sm:inline-block">
                  {user.displayName}
                </span>
                <button
                  id="signout-btn"
                  onClick={onGoogleSignOut}
                  className="text-[9px] bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/15 px-2 py-0.5 rounded transition"
                >
                  退出
                </button>
              </div>
          ...
          {!user ? (
            <div className="py-2 space-y-3">
              <p className="text-[11px] text-zinc-400 leading-relaxed font-normal">
                通过授权，SILI 可以在云端为你激活 Google Tasks 智能打卡、Google
                Calendar 自动排程、Google Docs 深度笔记本转录、Google Sheets
                数据表、Google Forms 评估表单，以及 Google Chat 团队广播提醒！
              </p>
            ...
            <div className="space-y-4">
                ...
                {[
                  ...
                  return (
                    <button
                      key={channelTab.id}
                      onClick={() =>
                        setActiveIntegrationChannel(channelTab.id as any)
                      }
                ...
                <div className="space-y-4 animate-in fade-in duration-200">
                  ...
                  {activeWidgetTab === "sync" ? (
                    <div className="space-y-3">
                      ...
                      <div className="bg-black/20 p-3 rounded-xl border border-white/[0.02] flex justify-between items-center text-[11px]">
                        <div className="space-y-1">
                          ...
                          <div className="font-mono text-zinc-500 leading-none">
                            SILI: {plan.goal.substring(0, 25)}...
                          </div>
                        </div>
                        <div className="text-right space-y-1">
                          <div className="font-semibold text-apple-blue">
                            已链接进度
                          </div>
                          <div className="font-mono text-zinc-400">
                            {stats.syncedCount} / {stats.totalUnits} 颗微突触
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <form
                        onSubmit={handleCreateBrowserTask}
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setNewBrowserTaskText(e.target.value)
                          }
                        ...
                        <button
                          type="submit"
                          disabled={
                            isWidgetLoading || !newBrowserTaskText.trim()
                          }
                          ...
                          browserTasks.map((bt) => (
                            ...
                            >
                              <div className="flex items-center gap-2.5 min-w-0">
                                ...
                                <span
                                  className={`text-[11px] font-medium leading-none truncate ${bt.status === "completed" ? "text-zinc-500 line-through" : "text-zinc-250"}`}
                                >
                                  {bt.title}
                                </span>
                              </div>
                            ...
                            >
                              <div className="min-w-0 flex items-start gap-2.5">
                                <div className="text-center bg-apple-blue/10 px-2 py-1 rounded border border-apple-blue/15 scale-95 origin-top-left">
                                  <div className="text-[8px] text-zinc-500 font-bold leading-none uppercase">
                                    {dateStr}
                                  </div>
                                  <div className="text-[10px] text-apple-blue font-bold font-mono mt-0.5 leading-none">
                                    {timeStr}
                                  </div>
                                </div>
                ...
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="bg-black/20 p-4 border border-white/5 rounded-xl space-y-2">
                    ...
                    <p className="text-[10px] text-zinc-400 leading-normal">
                      一键快速建立并将当前的宏观战略里程、微日志完成比例以及突触统计，渲染为一篇逻辑清晰排版优雅的
                      Google Docs 文件存放在主云盘！
                    </p>
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const confirmed = window.confirm(
                                    "确定要在本地历史记录中删除此篇 Docs 指引吗？(不会影响云端实体文档)",
                                  );
                                  if (confirmed) {
                                    setCreatedDocs((prev) =>
                                      prev.filter((d) => d.id !== docItem.id),
                                    );
                                  }
                                }}
              ...
              {activeIntegrationChannel === "sheets" && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  ...
                  {createdSheets.length > 0 && (
                    <form
                      onSubmit={handleAddHoursRowToSheets}
                      className="bg-black/20 p-4 border border-white/5 rounded-xl space-y-3"
                    >
                      ...
                      <div className="space-y-2">
                        <div>
                          <label className="block text-[9px] text-zinc-500 font-bold mb-1">
                            今日的心得/专注主题
                          </label>
                          <input
                            type="text"
                            ...
                            onChange={(e) =>
                              setNewSheetLogContent(e.target.value)
                            }
                          ...
                          >
                            ...
                            <div className="flex gap-1.5 shrink-0">
                              ...
                              <button
                                onClick={() => {
                                  const confirmed = window.confirm(
                                    "确定要在本地历史记录中删除此 Sheets 数据表格吗？(不会影响云端实体表格)",
                                  );
                                  if (confirmed) {
                                    setCreatedSheets((prev) =>
                                      prev.filter((d) => d.id !== sheetItem.id),
                                    );
                                  }
                                }}
              ...
              {activeIntegrationChannel === "forms" && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="bg-black/20 p-4 border border-white/5 rounded-xl space-y-2">
                    ...
                    <p className="text-[10px] text-zinc-400 leading-normal">
                      运用 Google Forms
                      API，一键云端生成与当前进化目标深度挂钩的学习复盘问卷。包含满意度评价、阻碍困难收集、后续加强训练方向分析等维度。
                    </p>
                          ...
                          >
                            ...
                            <div className="flex gap-1.5 shrink-0">
                              ...
                              <button
                                onClick={() => {
                                  const confirmed = window.confirm(
                                    "确定要在本地历史记录中删除此表单吗？(不会影响云端表单)",
                                  );
                                  if (confirmed) {
                                    setCreatedForms((prev) =>
                                      prev.filter((f) => f.id !== formItem.id),
                                    );
                                  }
                                }}
                      ...
                      >
                        {chatSpaces.map((sp) => (
                          <option key={sp.name} value={sp.name}>
                            {sp.displayName} ({sp.type === "ROOM" ? "室" : "企"}
                            )
                          </option>
                        ))}
                      </select>
                ...
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="bg-orange-500/10 border border-orange-500/15 p-2 rounded-xl">
                    <p className="text-[8px] text-orange-400 font-semibold leading-relaxed leading-tight text-center">
                      📌 提示：由于消费者个人 Gmail 账号不受 GCP 团队开放 Keep
                      API 的直接授权，SILI 已自动为您安全降级为“本地精装 Keep
                      便签板”（基于 LocalStorage 极速闪存记录，并保留全功能）。
                    </p>
                  </div>

                  {/* Add sticky notes form */}
                  <form
                    onSubmit={handleAddKeepNote}
                    className="bg-black/20 p-2.5 rounded-xl border border-white/[0.02] space-y-2"
                  >
                    <div className="flex gap-2">
                      <input
                        type="text"
        ...
        <div className="relative border-l border-zinc-800 ml-3 pl-6 space-y-8 py-2">
          {plan.macro.map((m, idx) => (
            <div key={idx} className="relative">
              ...
              <h4 className="text-sm font-bold text-zinc-200 mb-2 tracking-tight">
                {m.month}
              </h4>
              <ul className="space-y-1.5 ms-0.5">
                {m.milestones.map((ms, j) => (
                  <li
                    key={j}
                    className="flex items-start gap-2 text-xs text-zinc-400 leading-normal group"
                  >
      ...
      >
        ...
        >
          <h3 className="text-[15px] font-bold tracking-tight text-white flex items-center gap-3">
            ...
            微观落实任务单
            <i
              className={`fas fa-chevron-${isTasksCollapsed ? "up" : "down"} text-zinc-500 text-[11px] ml-2 group-hover:text-white transition-colors`}
            ></i>
          </h3>
          <span className="text-[10px] bg-apple-blue/15 text-apple-blue px-3.5 py-1.5 rounded-full font-bold uppercase tracking-wider border border-apple-blue/20">
            待执行 {stats.remainingUnits} 周期
          </span>
        </div>

        {!isTasksCollapsed && (
          <div className="flex-1 overflow-hidden flex flex-col px-6 md:px-10 pb-8">
            <p className="text-[11px] text-zinc-400 leading-normal mb-5 px-1 shrink-0">
              点击开启内置番茄计时。每个学习周期持续30分钟，深度锻炼绝对专注力。
            </p>
            ...
            <div className="mt-6 pt-5 border-t border-white/10 shrink-0">
              <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-wider">
                ...
                <span className="text-apple-blue font-mono">
                  {Math.round(
                    (stats.completedUnits / (stats.totalUnits || 1)) * 100,
                  )}
                  %
              ...
              <div className="h-[4px] w-full bg-zinc-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(stats.completedUnits / (stats.totalUnits || 1)) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
