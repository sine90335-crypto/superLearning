...
export const Editor: React.FC<EditorProps> = ({ note, onSave, onGenerateCards, onGenerateQuiz, onStartDemo, onImport, isGenerating }) => {
  ...
  const triggerAutoSave = (currentTitle: string, currentContent: string) => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      onSave({ ...note, title: currentTitle, content: currentContent, updatedAt: Date.now() });
      setSaveStatus('已保存');
  ...
  return (
    <div className="flex flex-col h-full bg-[#1c1c1e]/40 backdrop-blur-md rounded-2xl border border-white/5 overflow-hidden shadow-2xl">
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <button 
            ...
            className="px-3.5 py-1.5 bg-[#2c2c2e] hover:bg-[#3a3a3c] text-zinc-300 disabled:opacity-40 rounded-xl text-xs font-semibold tracking-tight transition flex items-center gap-1.5 border border-white/5"
          >
            <i className="fas fa-play-circle text-xs text-purple-400"></i>模拟演练
          </button>
          <button 
            disabled={isGenerating}
        ...
        <div className="flex-1 p-6 overflow-y-auto bg-transparent text-left markdown-renderer border-t border-white/5 bg-black/5 custom-scrollbar">
          {content ? (
            ...
            <div className="text-center py-20 text-zinc-500 font-medium text-xs leading-relaxed space-y-2">
              ...
              <p>当前笔记中尚无内容...</p>
              <p className="text-[10px] text-zinc-650">请切换至“编辑源码”开始勾勒您的知识脉络！</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="border-t border-white/5 bg-[#161617]/10 p-3.5 transition-all">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-indigo-505 animate-pulse" style={{ backgroundColor: '#6366f1' }}></span>
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest flex items-center gap-1">
              <i className="fas fa-link text-indigo-400"></i> Obsidian 知识库智连
            </span>
          </div>
          <button 
            type="button"
          ...
          >
            {showObsidianPanel ? "收起面板 (Close Panel)" : "配置与导出 (Connect & Export)"} <i className={`fas ${showObsidianPanel ? 'fa-chevron-down' : 'fa-chevron-up'} ml-1 text-[8px]`}></i>
          </button>
        </div>
        ...
        {showObsidianPanel && (
          <div className="mt-3.5 grid grid-cols-1 md:grid-cols-2 gap-4 bg-black/35 p-3.5 rounded-xl border border-white/5 animate-in slide-in-from-bottom duration-200">
            <div className="space-y-1.5 text-left">
              <label className="block text-[9px] text-zinc-500 font-bold uppercase tracking-wider">本地 Obsidian Vault 库名称</label>
              <input
                type="text"
                value={vaultName}
                onChange={(e) => {
              ...
              <p className="text-[9px] text-zinc-500 leading-normal">
                设置专属 Vault 后，下方深度协议唤醒接口将直连对应的本地笔记本体系。
              </p>
            </div>
              ...
              <div className="flex flex-wrap gap-1.5 justify-end">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    alert(`链接成功拷贝至剪贴板！可在浏览器中直接调用或配合 Obsidian 使用。\n\n链接: ${protocolLink}`);
                  }}
                ...
                >
                  <i className="fas fa-download text-[9px]"></i> 导出 .md 到 Obsidian 库
                </button>
              </div>
            </div>
          </div>
      ...
      {saveStatus && (
        <div id="editor-save-status" className="px-6 py-2 border-t border-white/5 bg-[#161617]/20 flex items-center justify-between text-xs font-mono text-zinc-500 transition-all duration-300">
          <div className="flex items-center gap-2">
            {saveStatus === '自动保存中...' ? (
              <>
                ...
                <span className="text-apple-orange font-medium text-[11px]">{saveStatus}</span>
              </>
            ) : (
              <>
                <span className="inline-flex rounded-full h-1.5 w-1.5 bg-apple-green"></span>
                <span className="text-apple-green font-medium text-[11px]">{saveStatus}</span>
              </>
            )}
          </div>
          {lastSavedTime && (
